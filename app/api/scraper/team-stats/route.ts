...
const BP_TEAM_MAP: Record<number, string> = {
  1: "FaZe Vegas", 2: "Los Angeles Thieves", 3: "Vancouver Surge", 4: "OpTic Texas",
  6: "Boston Breach", 11: "G2 Minnesota", 12: "Toronto KOI", 26: "Carolina Royal Ravens",
  27: "Miami Heretics", 62: "Riyadh Falcons", 63: "Cloud9 New York", 743: "Paris Gentle Mates",
};
...
export async function POST() {
  try {
    ...
    for (const t of teams) {
      ...
      const teamRes = await sb(`Teams?name=eq.${encodeURIComponent(teamName)}&select=id`);
      ...
      const statsData = {
        team_id: teamId,
        hardpoint_win_percent: (t.hp_map_win_percentage ?? 0) / 100,
        search_destroy_win_percent: (t.snd_map_win_percentage ?? 0) / 100,
        overload_win_percent: (t.ovl_map_win_percentage ?? 0) / 100,
        control_win_percent: 0,
        ...
        last_roster_change_date: new Date().toISOString(),
        recent_form_modifier: 1.0,
      };
      ...
      if (existing.length > 0) {
        ...
        const insertRes = await sb("TeamStats", {
          method: "POST",
          headers: { ...SB_HEADERS, Prefer: "return=representation" },
          body: JSON.stringify(statsData),
        });
        if (!insertRes.ok) {
          ...
          console.error("TeamStats insert failed:", err, "Data:", JSON.stringify(statsData));
        }
      }
      synced++;
    }
